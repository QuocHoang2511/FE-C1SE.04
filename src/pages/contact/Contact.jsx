...
const [name1,setname1] = useState("")
  ...
  useEffect(() => {
    if (auth) {
      auth?.fullName && setValue("name", auth.fullName);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.success("Gửi lời nhắn thành công, quản trị viên sẽ sớm liên hệ với bạn", {
        position: "top-right",
        autoClose: 5000,
      ...
      <div class="grid grid-cols-3 gap-4 px-24 mx-20 mb-20 ">
        <div class=" flex mx-10 justify-center items-center ">
          <div>
            <FontAwesomeIcon
              icon={faPhoneVolume}
              className="w-[40px] h-[40px] text-primaryColor"
            />
          </div>
